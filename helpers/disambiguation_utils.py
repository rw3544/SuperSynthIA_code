...
class CoordinateTransform(object):
    ...
    """
    def __init__(self, azimuth, field, inclination, keys, disambigAdd, sunpyMap):
        self.azimuth = azimuth
        self.field = field
        self.inclination = inclination
        self.keys = keys
        self.disambigAdd = disambigAdd
        self.sunpyMap = sunpyMap
    ...
    def ccd(self):
        ...
        size_field  = self.field[1].data.shape
        size_inclination = self.inclination[1].data.shape
        size_azimuth = self.azimuth[1].data.shape
        ...
        psi    =  (self.azimuth[1].data+self.disambigAdd)*(np.pi/180.)
...
#                coord_y[pix_x, pix_y] = pix_y + 1
                #SIGH
                coord_x[pix_y, pix_x] = pix_x + 1
                coord_y[pix_y, pix_x] = pix_y + 1


        try:
            x1 = (((coord_x-self.keys['CRPIX1'])*np.cos(self.keys['CROTA2']*(np.pi/180.)) - (coord_y-self.keys['CRPIX2'])*np.sin(self.keys['CROTA2']*(np.pi/180.)))*self.keys['CDELT1']+self.keys['CRVAL1'])
            y1 = (((coord_y-self.keys['CRPIX2'])*np.cos(self.keys['CROTA2']*(np.pi/180.)) + (coord_x-self.keys['CRPIX1'])*np.sin(self.keys['CROTA2']*(np.pi/180.)))*self.keys['CDELT1']+self.keys['CRVAL2'])
        except:
            pdb.set_trace()

        # step 2: populate these values into an astropy coordinate frame

        #hmimap = sunpy.map.Map(self.field[1].data, dict(self.keys['iloc']))
        c = SkyCoord(x1 * u.arcsec, y1 * u.arcsec, frame='helioprojective', rsun=self.keys['RSUN_REF'] * u.meter, observer=self.sunpyMap.observer_coordinate)
        ...
        b = self.keys['CRLT_OBS'] * (np.pi/180.)   # b-angle, disk center latitude, in radians
        p = - self.keys['CROTA2'] * (np.pi/180.)   # p-angle, negative of CROTA2, in radians
...
class CoordinateTransformMapPlusDisambigArrays(object):
    """
    Class for coordinate transformations.
    ...
    keys: pandas dataframe
        The relevant WCS keywords
    """
    def __init__(self, azimuth, field, inclination, keys, sunpyMap):
        self.azimuth = azimuth
        self.field = field
        self.inclination = inclination
        self.keys = keys
    ...
    def ccd(self):
        ...
        size_field  = self.field.shape
        size_inclination = self.inclination.shape
        size_azimuth = self.azimuth.shape
        if(size_field != size_inclination != size_azimuth):
            print("The three components are not of the same dimensions")
            print("Field",size_field,"inclination",size_inclination,"azimuth",size_azimuth)
            sys.exit(1)    
        ...
        psi    =  (self.azimuth)*(np.pi/180.)

        # convert to b_xi, b_eta, and b_zeta as defined in Equation 1 of Sun et al. 2013
        b_xi   = -1*self.field * np.sin(gamma) * np.sin(psi)
        b_eta  = self.field * np.sin(gamma) * np.cos(psi)
        b_zeta = self.field * np.cos(gamma)

        # --- convert from helioprojective cartesian to stonyhurst heliographic coordinates ---

        # step 1: convert from pixels to helioprojective cartesian coordinates
        coord_x = np.ndarray([self.keys['NAXIS2'], self.keys['NAXIS1']])
        coord_y = np.ndarray([self.keys['NAXIS2'], self.keys['NAXIS1']])

        for pix_x in range(self.keys['NAXIS2']):                         # data[1].header gives the uncompressed header
            for pix_y in range(self.keys['NAXIS1']):
#                coord_x[pix_x, pix_y] = pix_x + 1
#                coord_y[pix_x, pix_y] = pix_y + 1
                #SIGH
                coord_x[pix_y, pix_x] = pix_x + 1
        ...
        bptr = np.ndarray([self.keys['NAXIS2'], self.keys['NAXIS1'], 3])
        bptr[:,:,0] = k31 * b_xi + k32 * b_eta + k33 * b_zeta
        bptr[:,:,1] = k21 * b_xi + k22 * b_eta + k23 * b_zeta
        bptr[:,:,2] = k11 * b_xi + k12 * b_eta + k13 * b_zeta
